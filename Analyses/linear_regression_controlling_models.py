...
def predict_roi_rdm(train_x, train_y, test_x): # train_x: list(#seg, array(#layer, #time)), train_y: list(#seg, array(#layer, 1))
    ...
    for seg in range(len(train_x)):
        ...
        # print('seg_train_x:', seg_train_x.shape)
        # print('seg_train_y:', seg_train_y.shape)
        # print()
        ...
        seg_residual = np.subtract(seg_train_y, lr.predict(seg_train_x))
        # print('predict train_x:', lr.predict(seg_train_x).shape)
        # print('seg residual shape:', seg_residual.shape)
        train_residuals.append(seg_train_y - lr.predict(seg_train_x))
        # print()
    ...
    return pred_test_y, train_residuals, betas, intercepts

def rdm_linear_regression(train_layer_rdms, train_subject_rdms, test_layer_rdms, test_subject_rdms):
    pred_test_subject_rdm, train_residuals, betas, intercepts = predict_roi_rdm(train_layer_rdms, 
                                                                                train_subject_rdms, 
                                                                                test_layer_rdms)
    corr = pearson_corr(pred_test_subject_rdm, test_subject_rdms)
    return pred_test_subject_rdm, train_residuals, corr, betas, intercepts
...
def setup_data(subject, 
    ...
    return train_layer_rdms, train_subject_rdms, test_layer_rdms, test_subject_rdms
...
pairwise_control_corr = {}
# pairwise_betas = {}
do_break = False
for roi_id in range(len(roi_names)):
    print(roi_names[roi_id])
    pairwise_control_corr[roi_names[roi_id]] = np.zeros((n_models, n_models))
    # pairwise_betas[roi_names[roi_id]] = {}
    m1 = 0
    for model1 in dnn_models:
        for mode1 in modes[model1]:
            m2 = 0
            for model2 in dnn_models:
                for mode2 in modes[model2]:
                    if m1 == m2:
                        m2 += 1
                        continue
                    
                    model_names = [model1, model2]
                    model_modes = [mode1, mode2]
                    
                    print('control:', model1, mode1)
                    print('in:', model2, mode2)
                    corr = np.zeros((n_segments))
                    for s in range(n_segments):
                        train_layer_rdms, \
                        train_subject_rdms, \
                        test_layer_rdms, \
                        test_subject_rdms = setup_data(subject=subject, 
                        ...
                        control_layers_num = len(layer_names[model1][mode1])
                        control_train_model_rdms = [rdm[:control_layers_num, :, :] for rdm in train_layer_rdms]
                        control_test_model_rdms = test_layer_rdms[:control_layers_num, :, :]
                        
                        target_train_model_rdms = [rdm[control_layers_num:, :, :] for rdm in train_layer_rdms]
                        target_test_model_rdms = test_layer_rdms[control_layers_num:, :, :]
                        
                        control_model_prediction, \
                        train_residuals, \
                        _, \
                        _, \
                        _ = rdm_linear_regression(control_train_model_rdms, 
                                                  train_subject_rdms, 
                                                  control_test_model_rdms, 
                                                  test_subject_rdms)

                        test_residuals = np.subtract(test_subject_rdms.squeeze(), 
                                                     control_model_prediction)
                        ...
                        corr[s], \
                        betas, \
                        intercepts = rdm_linear_regression(target_train_model_rdms, 
                                                           train_residuals, 
                                                           target_test_model_rdms, 
                                                           test_residuals)
                    ...
                    print(corr_mean)
                    print()
                    pairwise_control_corr[roi_names[roi_id]][m1, m2] = corr_mean
                    m2 += 1
            m1 += 1

f = open(pjoin(subject_res_dir, '4_streams_linreg_controlling_model.pkl'),'wb')
pickle.dump(pairwise_control_corr, f)
f.close()

# f = open(pjoin(subject_res_dir, '4_streams_linreg_betas.pkl'),'wb')
# pickle.dump(pairwise_betas, f)
# f.close()