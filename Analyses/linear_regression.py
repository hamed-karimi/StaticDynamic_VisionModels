...
# categories = model_comb.keys()
# models_pcorr = {}
# pairwise_corr = np.zeros((len(roi_names), n_models, n_models))
pairwise_corr = {}
pairwise_betas = {}
for roi_id in range(len(roi_names)):
    print(roi_names[roi_id])
    pairwise_corr[roi_names[roi_id]] = np.zeros((n_models, n_models))
        ...
        for mode1 in modes[model1]:
            ...
            for model2 in dnn_models:
                for mode2 in modes[model2]:
                    if model1 == model2 and mode1 == mode2:
                        model_names = [model1]
                        model_modes = [mode1]
                    else:
                    ...
                    # run a lin reg using all the layer rdms of model_names to
                    # estimate beta values for each run, then avg the beta values to 
                    # predict the neural rdm of the 7-th run.
                    print(model_names)
                    print(model_modes)
                    print()
                    corr = np.zeros((8))
                    for s in range(8):
                        pred_rdm, corr[s], betas = predicted_segment_rdm(subject=subject, 
                    ...
                    pairwise_corr[roi_names[roi_id]][m1, m2] = corr.mean()
                    if model1 == model2 and mode1 == mode2:
                        pairwise_betas[roi_names[roi_id]][model1+'-'+mode1] = betas
                    m2 += 1
            m1 += 1
    # break
f = open(pjoin(subject_res_dir, '4_streams_linreg-pos-coef.pkl'),'wb')
pickle.dump(pairwise_corr, f)
f.close()

f = open(pjoin(subject_res_dir, '4_streams_linreg_betas-pos-coef.pkl'),'wb')
